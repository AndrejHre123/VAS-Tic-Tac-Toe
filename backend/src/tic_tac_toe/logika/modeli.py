...
from tic_tac_toe.logika.iznimke import InvalidMove, UnknownGameScore
from tic_tac_toe.logika.validatori import validate_game_state, validate_grid
...
class GameState:
    ...
    def make_move_to(self, index: int) -> Move:
        if self.grid.cells[index] != " ":
            raise InvalidMove("Polje nije prazno")
        return Move(
    ...
    def evaluate_score(self, mark: Mark) -> int:
        if self.game_over:
            ...
            if self.winner is mark:
                ...
                return -1
        raise UnknownGameScore("Igra još nije završila")